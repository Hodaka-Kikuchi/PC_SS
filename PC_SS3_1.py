...
class ScreenshotApp:
    def __init__(self, root):
        ...
        self.selected_window_title = tk.StringVar(value="未選択")
        self.running = False
        ...
        tk.Label(root, text="対象アプリケーション:").grid(row=1, column=0, sticky="w")
        tk.Entry(root, textvariable=self.selected_window_title, width=50, state="readonly").grid(row=1, column=1)
        tk.Button(root, text="アプリ選択", command=self.select_window).grid(row=1, column=2)

        tk.Label(root, text="スクリーンショット間隔 (秒):").grid(row=2, column=0, sticky="w")
        tk.Entry(root, textvariable=self.screenshot_interval, width=10).grid(row=2, column=1, sticky="w")
    ...
    def select_window(self):
        messagebox.showinfo("ウィンドウ選択", "対象のアプリケーションウィンドウをクリックしてください。")
        selected_window = self.get_window_title_on_click()
        if selected_window:
            self.selected_window_title.set(selected_window)
    ...
    def start_screenshots(self):
        ...
        if not self.selected_window_title.get() or self.selected_window_title.get() == "未選択":
            messagebox.showerror("エラー", "アプリケーションウィンドウを選択してください。")
            return
    ...
    def screenshot_loop(self, interval):
        try:
            while self.running:
                ...
                #output_path = os.path.join(self.save_folder.get(), f"screenshot_{timestamp}.bmp")
                output_path = os.path.join(self.save_folder.get(), f"screenshot1.bmp")
                if not self.capture_window(self.selected_window_title.get(), output_path):
